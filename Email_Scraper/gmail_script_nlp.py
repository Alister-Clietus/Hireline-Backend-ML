...
from nlp_extractor import extract_all_info  # Import the NLP extractor
...
def save_email_to_db(cursor, connection, mail_id, subject, sender_id, content, links, extracted_info):
    """Save email data to the MySQL database."""
    insert_query = f"""
    INSERT INTO {TABLE_NAME} 
    (mail_id, subject, sender_id, content, links, company_name, job_title, lpa_provided, contact_details, bond_info)
    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s);
    """
    # Convert 'contact_details' to JSON format
    contact_details_json = json.dumps(extracted_info['contact_details'])
    ...
    cursor.execute(insert_query, (
        ...
        extracted_info['job_title'],
        extracted_info['lpa_provided'],
        contact_details_json,  # JSON object instead of string
        extracted_info['bond_info']
    ))
    connection.commit()
...
def get_emails_with_label(service, label_name, cursor, connection):
    ...
    for message in messages:
        ...
        extracted_info = extract_all_info(content, subject)

        # Print the extracted details
        print("\nExtracted Details:")
        print(f"Company Name: {extracted_info['company_name']}")
        print(f"Job Title: {extracted_info['job_title']}")
        print(f"LPA Provided: {extracted_info['lpa_provided']}")
        print(f"Contact Details: {extracted_info['contact_details']}")
        print(f"Bond Info: {extracted_info['bond_info']}")
        print("-" * 40)

        # Save to the database
        save_email_to_db(cursor, connection, msg['id'], subject, sender_id, content, links_str, extracted_info)
